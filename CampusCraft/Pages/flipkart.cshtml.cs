...
{
    public class flipkartModel : PageModel
    {

        [BindProperty]
        public required flipkart Reg { get; set; }
        ...
        public void OnPost()
        {
            ...
            using (MySqlConnection con = new MySqlConnection("server=localhost;username=root;database=campuscraft;port=3306;password=rohan@3136"))
            {
                ...
                comm.CommandText = "INSERT INTO flipkart(name,email,collegeid,12per,graper,gender,cover) VALUES(?name, ?email, ?idnumber, ?per12, ?pergra, ?gender, ?cover)";
